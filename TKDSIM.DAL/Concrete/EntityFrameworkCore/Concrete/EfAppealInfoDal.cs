                ...
                if (item.OrderStatus != 0)
                {
                    var resultOrder = from a in context.AppealInfos
                                      ...
                                      select new AppealInfoDTO
                                      {
                                          ...
                                          AppealTypeName = a.ApplicantName,
                                          ...
                                          DeqkisNo = a.DeqkisNo,
                                          GrandAreaSize = a.GrandAreaSize,
                                          GrandCategory = a.GrandCategory,
                                          IED_EV = a.IED_EV,
        ...
        public async Task<List<ReportDto>> Report(int id)
        {
            using (var context = new TKDSIMDBContext())
            {
                ...
                var result = from a in context.AppealInfos
                             ...
                             join evGrandCategory in context.EnumValues on a.GrandCategory equals evGrandCategory.EV_ID into evGrandCategorytemp
                             from evGrandCategory in evGrandCategorytemp.DefaultIfEmpty()
                             join evWillChangecat in context.EnumValues on a.WillChangeCategory equals evWillChangecat.EV_ID into evWillChangecattemp
                             from evWillChangecat in evWillChangecattemp.DefaultIfEmpty()
                             join evAppealType in context.EnumValues on a.AppealType equals evAppealType.EV_ID into evAppealTypetemp
                             from evAppealType in evAppealTypetemp.DefaultIfEmpty()
                             join evPropertType in context.EnumValues on a.PropertType equals evPropertType.EV_ID into evPropertTypetemp
